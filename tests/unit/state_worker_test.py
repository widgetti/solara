...
def test_breaker_resolve_probe_unwedges_half_open():
    # a caller that consumes the probe via allow() but reports neither success nor failure
    # (a NOTHING/DISABLED flush outcome) must not leave the breaker wedged half_open forever.
    clock = FakeClock()
    breaker = CircuitBreaker(failures=1, window=30.0, clock=clock)
    breaker.record_failure()  # open
    clock.advance(30.0)
    assert breaker.allow()  # consumes the single probe
    assert breaker.state == "half_open"
    assert not breaker.allow()  # probe outstanding -> wedged without a resolution
    breaker.resolve_probe()  # neutral resolution
    assert breaker.state == "closed"
    assert breaker.allow()


def test_breaker_resolve_probe_is_noop_when_closed():
    # resolve_probe must not disturb closed-state consecutive-failure accounting
    breaker = CircuitBreaker(failures=2, window=30.0, clock=FakeClock())
    breaker.record_failure()  # 1 of 2, still closed
    breaker.resolve_probe()  # no probe outstanding -> must be a no-op
    assert breaker.state == "closed"
    breaker.record_failure()  # 2 of 2 -> opens (proves the failure count was NOT reset)
    assert breaker.state == "open"
...
def test_flush_skips_oversize_value_keeps_the_rest(monkeypatch):
    # §4.3 size guard: one oversize reactive is skipped (not flushed) but does NOT disable the
    # kernel or lose the other keys, and the drop is counted.
    monkeypatch.setattr(solara.server.settings.state, "max_value_bytes", 1000)
    stats()._reset()
    small = solara.reactive("ok", persist=True, key="test.worker.small")
    big = solara.reactive("x", persist=True, key="test.worker.big")
    backend = MemoryStateBackend()
    context = make_context("oversize-kernel")
    manager = attach_manager(context, backend)
    with context:
        small.value = "keep-me"
        big.value = "y" * 5000  # envelope well over the 1000-byte cap
    outcome = manager.flush_now()
    assert outcome == FlushOutcome.OK
    assert stats().sync_oversize_dropped == 1
    # the small key persisted; the oversize key was not written
    stored = backend._store[context.id].fields
    assert persist.FIELD_PREFIX + "test.worker.small" in stored
    assert persist.FIELD_PREFIX + "test.worker.big" not in stored
