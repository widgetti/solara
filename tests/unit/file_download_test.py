...
def test_download(tmpdir: Path):
    ...
    assert rc.find(FileDownloadWidget).widget.bytes == b"test content"
    assert rc.find(Btn).widget.children[0] == "Download: solara-download.dat"
    ...
    assert rc.find(FileDownloadWidget).widget.bytes is None
    rc.find(FileDownloadWidget).widget.request_download = True
    assert rc.find(FileDownloadWidget).widget.bytes == b"test content2"
    ...
    assert rc.find(Btn).widget.children[0] == "Download: test.txt"
    rc.find(FileDownloadWidget).widget.request_download = True
    assert rc.find(FileDownloadWidget).widget.bytes == b"test content3"
    ...
    assert rc.find(FileDownloadWidget).widget.bytes == b"lazy read"
