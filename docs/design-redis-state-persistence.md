...
dev/test-gated `evict` method on the `solara.control` comm) evict the kernel from
`contexts` first, so the reconnect behaves exactly like landing on a fresh instance —
with only backend state surviving. The evict goes over the kernel **websocket**, not
HTTP: behind a round-robin load balancer an HTTP evict lands on an arbitrary instance
and 404s on every non-owner, while the websocket by definition terminates on the
instance that owns the kernel (this also makes the debug hook server-agnostic — flask
included — and needs no cookie check, since the comm only exists on the kernel's own
authenticated connection; the HTTP route remains for out-of-band tooling). Crucially,
