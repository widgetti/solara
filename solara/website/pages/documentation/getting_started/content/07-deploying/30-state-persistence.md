...
Generate a strong secret key (do this **once** and store it in your secrets manager — every
instance in the fleet must use the *same* value):
...
1. **Add-new-verify-only** — generate a new key
   (`python -c "import secrets; print(secrets.token_urlsafe(32))"`) and **prepend** it, keeping the
   old: `SOLARA_STATE_SECRET_KEYS="new-secret,old-secret"`. New writes are signed with `new-secret`;
...
Restores are rotation-safe end to end: state written under the old key still authenticates against
the new key set (verify-any) and is migrated to the new primary on its next write, so promoting a
new key does **not** orphan in-flight sessions.
