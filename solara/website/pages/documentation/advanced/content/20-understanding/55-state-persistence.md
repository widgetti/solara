...
# generate a strong secret once (same value on every instance); REQUIRED whenever a backend is enabled
export SOLARA_STATE_SECRET_KEYS="$(python -c 'import secrets; print(secrets.token_urlsafe(32))')"
```
...
### Tightening JSON decode (optional)

The default JSON codec restores tagged dataclasses / pydantic models / enums by resolving the
class named in the (HMAC-verified) envelope, gated to that kind. If you want a stricter boundary —
so that even an attacker holding the secret can only reconstruct types you explicitly allow — call
`solara.state.allow_decode_types("myapp.models:Renter", "myapp.models:Unit", ...)` at startup.
Once any type is registered the allow-list is enforced; leaving it empty keeps the default
(kind-gated) behavior, which breaks nothing.
...
### Dependent selections: the orphaned-selection trap

A selection that indexes into a *derived* list — Continent → Country → City — is where
`persist=True` on the wrong reactive produces the subtlest failure: a restore that **reports
success and is still wrong**.
...
restore installs `country = "France"` — a value not in the list it was chosen from. Depending on
the consumer you get a select rendering a **phantom** value absent from its items, an
`options.index(country.value)` raising `ValueError`, or a widget that validates/clamps on mount
and **silently rewrites the persisted value**. `lastRestore.status` is `"success"` throughout —
the persistence layer did its job; the *set of keys you chose* was not self-consistent.

**The principle:** persisted keys are restored onto a blank kernel **together, and alone.**
Persisting a selection obligates you to (a) make the set it selects from reconstructible on that
blank kernel, and (b) re-validate the selection against the *recomputed* set on restore (the new
instance may also see newer data). A decision ladder:

1. **Persist the leaf identifier only; derive the parents upward.** If the leaf has a stable
   unique id, `country`/`continent` are a pure lookup *from* it, so a restore cannot be
   internally inconsistent — and it is *fewer* persisted keys, not more:
   ...
   This is the opposite of the reflex "then I must persist the continent too… and the list… and
   it recurses." Persisting the whole chain works but is usually the worst fix: each key flushes
   on its own [debounce window](#configuration-reference), so a continent changed at `t` and a
   country picked at `t+2s` are separate writes — a failover between them restores a *torn* pair —
   and you still owe the re-validation. Never persist a selection as an object embedding its
   parents (a stale copy), nor as a list *index* (drifts when the list reorders).

2. **When upward derivation is impossible,** persist the **top input** and recompute + re-validate
   downward — reset an orphaned value, never render it:
   ...
   The guard lives in the component (not in a restore hook) because an orphaned selection is not
   failover-specific — any data refresh can invalidate a list under its selection.

3. **Never persist the derived list** (`country_options`) — recompute it; a persisted copy can
   disagree with what the new instance computes from fresh data, and it may be large.

**"But the derivation is expensive."** Persisting the expensive *output* is still the wrong
answer — a failover can restore a stale output that disagrees with its recomputed inputs, with no
signal. Let it recompute on the new instance (a failover is a cold start, which already pays that
cost — the `examples/state-persistence-failover/` demo recomputes deliberately), or cache it in a
store **keyed on and invalidated by its inputs** (a cache, not `persist`). Persisting the output
*plus* a persisted hash of its inputs, discarded on mismatch, is a last resort — that is
hand-rolling the same cache inside the state envelope.

**Litmus test:** if you restore *only* the `persist=True` keys onto a blank kernel, is each one
valid on its own — or does it depend on a sibling you did not persist? If it names a sibling,
derive that sibling *from* the persisted key (rung 1) or persist the root input and add the
re-validation guard (rung 2).
...
| `SOLARA_STATE_BAILOUT_STORM_THRESHOLD` | `0.5` | Fraction of recent restores that may bail out before the refresh dialog is suppressed and the fleet degrades to a silent fresh start. |
| `SOLARA_STATE_WARN_VALUE_BYTES` | `1000000` | Serialized-envelope size (per variable) above which a flush logs a warning; still persisted. |
| `SOLARA_STATE_MAX_VALUE_BYTES` | `5000000` | Serialized-envelope size above which a value is **skipped** (not persisted); the rest of the session still flushes. `0` disables the cap. |
...
These non-`SOLARA_STATE_` settings also matter when persistence is enabled:

| Environment variable | Default | Meaning |
| - | - | - |
| `SOLARA_SESSION_HTTP_ONLY` | `False` | Make the session cookie unreadable to JavaScript. Recommended with persistence (the cookie is a state-recovery credential); the server warns if it is off. |
| `SOLARA_KERNEL_MAX_PER_SESSION` | `100` | Max live kernels one session cookie may create (bounds the client-chosen-`kernel_id` amplifier). Enforced only when a backend is configured; `0` disables. |
| `SOLARA_RESOURCEZ_TOKEN` | `""` | Bearer token that unlocks the un-redacted `/resourcez` per-key breakdown in production. Unset ⇒ production hashes the key labels. |
