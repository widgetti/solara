...
from solara.components import Code, MarkdownIt
import inspect
        ...
        with v.Tabs(v_model=tab, on_v_model=set_tab, vertical=True):
            for key in routes.keys():
                with v.Tab(children=[key]):
                    pass
            component = list(routes.values())[tab]
...
use_side_effect can be used to run a function after the widgets are created, to optionally perform so called 'side effects', while at the same time providing a function that will clean up any side effect.
Typical side effects include attaching event handlers, fetching data and starting threads or tasks.

Providing a cleanup function ensure our app can always dispose of resources when needed. If no dependencies are passed, the function will be executed after each render phase.
Before your effect function is called for the next time, the cleanup function will be called.
...
Lets create a Button again, like we did in `use_state`, but now we manually attach the click handler.
        ''')
            IncludeComponent(CustomClickEvent, '''
import ipywidgets as widgets

''', highlight=[13, 15])
            MarkdownIt("""
Note that we return a `cleanup` function, so `react-ipywidgets` knows how to remove the event handler again. Note that under water, `on_click` does exactly this for you, but with much less code.
Note that we use `get_widget` to get a reference to the real ipywidgets `Button` (this function can only be called from within an effect function).

### Long running jobs

If we want to run a long running job, we do not want to do that in the render loop, since the render loop can be executed multiple times. Also, doing work in the render loop will cause
our app to be unresponsive. 

A better way would be to include the long running task in a thread, which will be started by the side effect hook, as well as a cleanup function that will stop the execution of the work.

        """)
            IncludeComponent(LongRunning, '''
...
Since this pattern is so common, this funtionality is included in a user hook, called `use_thread`, making is much easier to include such long runnings tasks in your
components.
""")
            IncludeComponent(LongRunningEasier, '''
from solara.hooks.hooks import use_thread
...
routes = {
    "use_state": DocUseState,
    "use_effect": DocUseEffect,
    "use_download": DocUseDownload
}
