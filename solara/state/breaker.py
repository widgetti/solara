...
class CircuitBreaker:
    ...
    def resolve_probe(self, success: bool = True) -> None:
        """Resolve a half-open probe a caller consumed via :meth:`allow` but has no health signal for.

        ``allow()`` hands out exactly one probe and sets ``_probe_used``; a caller that then returns
        without ``record_success``/``record_failure`` (a flush that was a no-op or disabled itself,
        never a real backend health signal) would otherwise leave the breaker wedged HALF_OPEN and
        turn persistence off process-wide. This resolves that probe; it is a no-op unless a probe is
        actually outstanding, so it never distorts closed-state consecutive-failure accounting.
        """
        with self._lock:
            if self._state == HALF_OPEN and self._probe_used:
                self._transition(CLOSED if success else OPEN, "probe-resolved-neutral")
