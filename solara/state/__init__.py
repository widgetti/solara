...
def validate_state_settings() -> None:
    ...
    if not _server_settings.session.http_only:
        logger.warning(
            "A state backend is enabled but SOLARA_SESSION_HTTP_ONLY is false. The session cookie "
            "is a cross-instance state-recovery credential; set SOLARA_SESSION_HTTP_ONLY=true so it "
            "is not readable by JavaScript (defense-in-depth against XSS-driven state theft)."
        )
