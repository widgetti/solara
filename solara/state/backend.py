...
class StateBackend(abc.ABC):
    ...
    def takeover(self, kernel_id: str, session_hmacs: "Union[bytes, Sequence[bytes]]", schema_tag: str) -> TakeoverResult:
        ...
        Verify-then-bump-then-read as one atomic unit. ``session_hmacs`` is the verify-ANY set of
        acceptable session HMACs (one per configured secret key, primary first) - a match against
        ANY is accepted, so a stored identity written under an old key still authenticates after a
        ...
        - Key exists but stored session-HMAC is absent or matches NONE of `session_hmacs`: write
          nothing; return reason="identity-mismatch", generation=0 (no write rights granted).
        ...
        Known narrow limitation (§5.5): if two instances take over the SAME missing key at the
        same instant (same session, broken LB stickiness), both receive generation 1 and the
        second's create-on-missing flush is NOT fenced out (stored == claimed == 1), so they can
        last-writer-wins in the tiny pre-first-flush window without a REJECTED signal. Closing this
        fully would require either writing on a miss (violating "miss writes nothing" above) or a
        per-connection token (which would break legitimate cross-instance takeover), so it is left
        as a documented same-session edge; once either instance's generation advances, the
        superseded-while-connected detector (worker.py) catches the divergence.
        """
