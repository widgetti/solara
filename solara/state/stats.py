...
# Rolling window (last N restore attempts) behind the bail-out storm valve (design §4.3). Small
# and count-based so it is deterministic in tests and needs no time bookkeeping. The valve stays
# inert until at least MIN samples exist, so a single early bail-out cannot trip it.
_STORM_WINDOW = 50
_STORM_MIN_SAMPLES = 10
...
class Stats:
    ...
    def __init__(self) -> None:
        ...
        self.sync_oversize_dropped = 0  # §4.3 size guard: values skipped for exceeding max_value_bytes
        self._sync_by_key: Dict[str, List[int]] = {}
        self._sync_by_kernel: Dict[str, List[int]] = {}
        # bail-out storm valve (§4.3): the last _STORM_WINDOW restore-attempt outcomes (True =
        # bailed out). A systemic cause - a forgotten schema_tag bump, non-uniform secret_keys
        # across replicas after a bad deploy - bails out EVERY reconnecting session at once; the
        # valve detects that and lets the server degrade to a silent fresh start instead of a
        # fleet-wide refresh dialog.
        self._restore_window: Deque[bool] = collections.deque(maxlen=_STORM_WINDOW)

    def record_restore_attempt(self, bailed_out: bool) -> None:
        """Feed one restore-attempt outcome to the bail-out storm window."""
        with self._lock:
            self._restore_window.append(bailed_out)

    def bailout_storm_active(self, threshold: float) -> bool:
        """Whether the recent bail-out rate is at/above ``threshold`` (needs a minimum sample).

        Only meaningful for a context that already failed recovery: the valve turns that user's
        refresh dialog into a silent remount during a storm; it never affects a successful restore.
        """
        with self._lock:
            n = len(self._restore_window)
            if n < _STORM_MIN_SAMPLES:
                return False
            return (sum(self._restore_window) / n) >= threshold
    ...
    def as_dict(self, verbose: bool = False) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            return {
                ...
                "sync_oversize_dropped": self.sync_oversize_dropped,
    ...
    def _reset(self) -> None:
        ...
        with self._lock:
            ...
            self.sync_keys_dropped = 0
            self.sync_kernels_dropped = 0
            self.sync_oversize_dropped = 0
            self._sync_by_key = {}
            self._sync_by_kernel = {}
            self._restore_window.clear()
